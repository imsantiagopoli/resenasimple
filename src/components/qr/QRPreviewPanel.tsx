...
        // CORRECCIÓN CLAVE: Asegurar la sintaxis correcta del gradiente para html2canvas
        const gradientDirection = convertGradientDirection(config.background.gradient.direction);
        backgroundStyle = `background: linear-gradient(${gradientDirection}, ${config.background.gradient.start}, ${config.background.gradient.end}) !important;`;
      } else if (config.background.type === 'image' && backgroundImageBase64) {
        // CORRECCIÓN CLAVE: Asegurar todas las propiedades de la imagen de fondo con base64
        backgroundStyle = `
